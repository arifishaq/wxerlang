...
start_link(Location, BoardPid, Parent, ActiveBrush, InactiveBrush, SelectedBrush) ->
    wx_object:start_link(
      ?MODULE, [Location, BoardPid, Parent, ActiveBrush, InactiveBrush, SelectedBrush], []).

init([Location, BoardPid, Parent, ActiveBrush, InactiveBrush, SelectedBrush]) ->
...
paint_square(#{square_panel := Panel,
    ...
    wxDC:setBackground(DC, case Selected orelse UnderCheck of 
			       true -> SelectedBrush; 
			       false -> case Active of 
					    true -> ActiveBrush;
					    false -> InactiveBrush 
					end
			   end),
